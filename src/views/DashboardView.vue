<template>
  <main class="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-start pt-10 sm:justify-center sm:pt-0 gap-6 px-4 pb-20 sm:pb-0">
    ...
    <div v-if="wallet.isConnected" class="flex flex-col items-center gap-4 w-full max-w-md">
      <div class="text-center">
        ...
        <button
          class="mt-4 px-4 py-2 min-h-11 bg-red-700 hover:bg-red-600 rounded-lg text-sm font-medium transition-colors"
          @click="disconnect"
        >
          Disconnect
        </button>
    ...
    <div v-else class="flex flex-col items-center gap-2">
      <template v-if="hasWallet">
        <button
          class="px-6 py-3 min-h-11 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-semibold transition-colors disabled:opacity-50"
      ...
      <template v-else>
        ...
        <a
          ...
          class="px-6 py-3 min-h-11 flex items-center bg-orange-600 hover:bg-orange-500 rounded-xl font-semibold transition-colors"
        >
          Install MetaMask
        </a>
      </template>
    ...
    <nav v-if="wallet.isConnected" class="hidden sm:flex gap-4">
      <RouterLink
        to="/swap"
